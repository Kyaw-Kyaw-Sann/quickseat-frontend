...
function StaffShellContent({ children }: { children: ReactNode }) {
  ...
  return <div className="min-h-screen bg-[#0c0c0f] text-[var(--qs-text)]">
    ...
    <div className="min-w-0 lg:pl-64"><header className="sticky top-0 z-20 border-b border-[var(--qs-border)] bg-[rgba(17,17,20,0.96)] backdrop-blur-xl"><div className="flex min-h-16 min-w-0 items-center gap-2 px-3 sm:gap-3 sm:px-6 lg:px-8"><Button aria-controls="staff-mobile-navigation" aria-expanded={drawerOpen} aria-label="Open staff navigation" className="min-h-11 min-w-11 shrink-0 px-3 lg:hidden" onClick={() => setDrawerOpen(true)} variant="secondary">Menu</Button><div className="min-w-0 flex-1"><p className="truncate text-sm font-semibold">Cinema Operations</p><p className="hidden truncate text-xs text-[var(--qs-text-muted)] sm:block">Assigned cinema workspace</p></div><div className="flex shrink-0 items-center gap-1 sm:gap-3"><Badge className="hidden sm:inline-flex" tone="primary">STAFF</Badge><div className="hidden max-w-52 md:block"><StaffIdentity /></div><Button className="min-h-11 px-2 sm:px-3 lg:hidden" disabled={isSigningOut} onClick={() => void handleLogout()} variant="ghost">{logoutLabel}</Button></div></div></header><main className="min-w-0 px-3 py-5 sm:px-6 sm:py-6 lg:px-8 lg:py-8"><div className="mx-auto min-w-0 max-w-[100rem]">{children}</div></main></div>
